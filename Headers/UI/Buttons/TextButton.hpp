...
struct TextButton
{
    ...
    TextButton(SDL_Renderer* renderer, float nx, float ny, float nw, float nh, SDL_Color NewButtonColor, SDL_Color NewTextColor);

    void handleFrame(bool ManageHover, SDL_Window* window, const char* Text);
    bool detectMouseHover(SDL_Window* window);
    void handleClick(SDL_Window* window);
};

TextButton::TextButton(SDL_Renderer* renderer, float nx, float ny, float nw, float nh, SDL_Color NewButtonColor, SDL_Color NewTextColor){
    x = nx; y = ny; w = nw; h = nh;
...
void TextButton::handleFrame(bool ManageHover, SDL_Window* window, const char* Text){
    ...
    SDL_Surface* TextSurface = TTF_RenderText_Blended(TextFont, Text, strlen(Text), TextColor);
    SDL_Texture* TextTexture = SDL_CreateTextureFromSurface(ButtonRenderer, TextSurface);

    SDL_RenderTexture(ButtonRenderer, TextTexture, NULL, &buttonrect);
